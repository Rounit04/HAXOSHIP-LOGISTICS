...
@section('title', 'Edit Payment Into Bank')
    ...
    <div class="page-header">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 mb-0.5">Edit Payment Into Bank</h1>
                    <p class="text-xs text-gray-600">Update payment information</p>
                </div>
            </div>
            <a href="{{ route('admin.payments-into-bank.all') }}" class="px-5 py-2.5 rounded-xl border-2 border-gray-300 text-gray-700 font-semibold hover:bg-gray-50 transition text-sm flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            <div class="form-card p-6">
                ...
                <form method="POST" action="{{ route('admin.payments-into-bank.update', $payment['id']) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <select name="bank_account" id="bank_account" class="form-select" required>
                            ...
                            @foreach($banks as $bank)
                                <option value="{{ $bank['bank_name'] }} - {{ $bank['account_number'] }}" {{ old('bank_account', $payment['bank_account'] ?? '') == $bank['bank_name'] . ' - ' . $bank['account_number'] ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <select name="type" id="type" class="form-select" required>
                            ...
                            <option value="Credit" {{ old('type', $payment['type'] ?? '') == 'Credit' ? 'selected' : '' }}>Credit</option>
                            <option value="Debit" {{ old('type', $payment['type'] ?? '') == 'Debit' ? 'selected' : '' }}>Debit</option>
                        </select>
                        @error('type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="remark" id="remark" rows="3" class="form-textarea resize-none" placeholder="Enter payment remarks or notes" required>{{ old('remark', $payment['remark'] ?? '') }}</textarea>
                        @error('remark')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
