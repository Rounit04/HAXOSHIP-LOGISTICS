...
@section('title', 'All Payments Into Bank')
    ...
    <div class="page-header">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 mb-0.5">All Payments Into Bank</h1>
                    <p class="text-xs text-gray-600">View and manage all bank payments</p>
                </div>
            </div>
            <a href="{{ route('admin.payments-into-bank.create') }}" class="admin-btn-primary px-4 py-2 text-sm font-semibold">
                    ...
                    <tbody>
                        @foreach($payments as $payment)
                            <tr>
                                ...
                                <td>
                                    <span class="badge {{ ($payment['type'] ?? '') == 'Credit' ? 'badge-credit' : 'badge-debit' }}">
                                        {{ $payment['type'] ?? '-' }}
                                    </span>
                                </td>
                                <td>{{ $payment['category_bank'] ?? '-' }}</td>
                                ...
                                <td>
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('admin.payments-into-bank.edit', $payment['id']) }}" class="p-2 rounded-lg bg-blue-50 hover:bg-blue-100 transition text-blue-600">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                            </svg>
                                        </a>
                                        <form action="{{ route('admin.payments-into-bank.delete', $payment['id']) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this payment?');" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="p-2 rounded-lg bg-red-50 hover:bg-red-100 transition text-red-600">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
