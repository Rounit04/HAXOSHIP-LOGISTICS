...
@section('content')
    ...
    <div id="success-popup" class="success-popup" style="display: none;">
        ...
        <div class="flex-1">
            <p class="text-green-700 font-semibold text-sm" id="success-message">Country updated successfully!</p>
        </div>
    ...
    <div class="page-header">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 mb-0.5">Edit Country</h1>
                    <p class="text-xs text-gray-600">Update country information - {{ $country['name'] }}</p>
                </div>
            </div>
            <a href="{{ route('admin.countries.all') }}" class="px-5 py-2.5 rounded-xl border-2 border-gray-300 text-gray-700 font-semibold hover:bg-gray-50 transition text-sm flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            <div class="form-card p-6">
                ...
                <form id="countryForm" method="POST" action="{{ route('admin.countries.update', $country['id']) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="country_name" id="country_name" class="form-input" value="{{ $country['name'] }}" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="country_code" id="country_code" class="form-input" value="{{ $country['code'] }}" required maxlength="10">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="country_isd_no" id="country_isd_no" class="form-input" value="{{ $country['isd_no'] }}" required maxlength="10">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="country_dialing_code" id="country_dialing_code" class="form-input" value="{{ $country['dialing_code'] }}" placeholder="e.g., 0091, 001" maxlength="10">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="remark" id="remark" rows="3" class="form-textarea resize-none" placeholder="Any additional notes">{{ $country['remark'] ?? '' }}</textarea>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="status-toggle">
                            <label class="status-switch">
                                <input type="checkbox" name="status" id="status" {{ $country['status'] == 'Active' ? 'checked' : '' }}>
                                <span class="status-slider"></span>
                            </label>
                            <span class="text-sm font-medium text-gray-700" id="statusLabel">{{ $country['status'] }}</span>
                        </div>
                    </div>
        ...
        <div class="lg:col-span-1">
            <div class="form-card p-5 sticky top-6">
                ...
                <div class="space-y-3">
                    <div>
                        <p class="text-xs text-gray-500 mb-1">Country ID</p>
                        <p class="text-sm font-bold text-gray-900">#{{ $country['id'] }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 mb-1">Current Name</p>
                        <p class="text-sm font-bold text-gray-900">{{ $country['name'] }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 mb-1">Current Code</p>
                        <p class="text-sm font-bold text-gray-900">{{ $country['code'] }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 mb-1">Current ISD No.</p>
                        <p class="text-sm font-bold text-gray-900">{{ $country['isd_no'] }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 mb-1">Current Status</p>
                        <span class="status-badge {{ strtolower($country['status']) }}">
                            {{ $country['status'] }}
                        </span>
                    </div>
                </div>
        ...
        document.getElementById('countryForm').addEventListener('submit', function(e) {
            ...
            .then(result => {
                if (result.redirected) {
                    // Show success popup
                    showSuccessPopup('Country updated successfully!');
                    ...
                    if (result.data.success) {
                        showSuccessPopup(result.data.message || 'Country updated successfully!');
                        // Redirect after a short delay
                        setTimeout(() => {
                        ...
                        if (result.data.errors) {
                            ...
                            alert(result.data.message || 'An error occurred while updating the country.');
            ...
            .catch(error => {
                ...
                alert('An error occurred while updating the country. Please check the console for details.');
                submitButton.disabled = false;
                submitButton.innerHTML = originalButtonText;
            });
        });
