    ...
    <div class="page-header">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 mb-0.5">Create Blog</h1>
                    <p class="text-xs text-gray-600">Add new blog post with image</p>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="form-card p-6">
                ...
                <form method="POST" action="{{ route('admin.blogs.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="title" id="title" class="form-input" placeholder="Enter blog title" required value="{{ old('title') }}">
                        @error('title')
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="content" id="content" class="form-textarea" placeholder="Enter blog content">{{ old('content') }}</textarea>
                        @error('content')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="author" id="author" class="form-input" placeholder="Admin Agent" value="{{ old('author', 'Admin Agent') }}">
                        @error('author')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="published_at" id="published_at" class="form-input" value="{{ old('published_at', date('Y-m-d')) }}">
                        @error('published_at')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="status" id="status" class="form-select" required>
                            <option value="published" {{ old('status') == 'published' || old('status') === null ? 'selected' : '' }}>Published</option>
            ...
            <div class="form-card p-5 sticky top-6">
                ...
                <div class="space-y-3 text-xs text-gray-600">
                    <div class="flex items-start gap-2">
                        ...
                        <span>Blog Title should be descriptive and engaging</span>
                    </div>
                    <div class="flex items-start gap-2">
                        <div class="w-1.5 h-1.5 rounded-full bg-purple-600 mt-1.5 flex-shrink-0"></div>
                        <span>Upload high-quality images (max 2MB)</span>
                    </div>
                    <div class="flex items-start gap-2">
                        <div class="w-1.5 h-1.5 rounded-full bg-purple-600 mt-1.5 flex-shrink-0"></div>
                        <span>Published blogs will appear on the homepage</span>
                    </div>
                    <div class="flex items-start gap-2">
                        <div class="w-1.5 h-1.5 rounded-full bg-purple-600 mt-1.5 flex-shrink-0"></div>
                        <span>Draft blogs are saved but not visible to users</span>
                    </div>
                </div>
