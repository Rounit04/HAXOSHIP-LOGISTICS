...
@section('content')
    ...
    <div class="page-header">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 mb-0.5">All Services</h1>
                    <p class="text-xs text-gray-600">Manage and edit all services - Single, bulk, editable Status (Active/Inactive)</p>
                </div>
            </div>
            <a href="{{ route('admin.services.create') }}" class="admin-btn-primary px-5 py-2.5 text-sm">
    ...
    <div class="search-section">
        ...
        <form method="GET" action="{{ route('admin.services.all') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <!-- Service Name Search -->
            <div>
            ...
            <div class="flex items-end gap-2">
                ...
                @if($searchParams['search'] ?? '' || $searchParams['network'] ?? '' || $searchParams['status'] ?? '')
                    <a href="{{ route('admin.services.all') }}" class="px-4 py-2.5 rounded-lg border-2 border-gray-300 text-gray-700 font-semibold hover:bg-gray-50 transition text-sm">
                        Clear
                    </a>
                @endif
            </div>
    ...
    <div class="service-card p-6">
        ...
        <!-- Professional Error Modal Popup -->
        @if(session('error'))
            @php
                    ...
                    <div class="flex-1 overflow-hidden flex flex-col bg-gray-50 min-h-0">
                        ...
                        <div class="bg-red-50 border-b border-red-200 px-5 py-3 flex-shrink-0">
                            <div class="flex items-center gap-2">
                                ...
                                <p class="text-sm font-semibold text-red-800">No services were imported. All errors must be resolved before importing.</p>
                            </div>
                        </div>
            ...
            <script>
                (function() {
                    ...
                    document.addEventListener('DOMContentLoaded', function() {
                        ...
                        // Close on outside click (only on modal backdrop)
                        ...
                        // Update visible count on scroll (throttled)
                        const scrollContainer = document.getElementById('errorScrollContainer');
                        if (scrollContainer) {
        ...
        <form id="bulkDeleteForm" method="POST" action="{{ route('admin.services.bulk-delete') }}">
            @csrf
            <div class="overflow-x-auto rounded-lg border border-gray-200">
                <table class="service-table min-w-full">
                    <thead>
                        <tr>
                            <th class="w-12">
                    ...
                    <tbody>
                        @forelse($services as $service)
                            <tr>
                                <td>
                                    <input type="checkbox" name="selected_ids[]" value="{{ $service['id'] }}" class="row-checkbox w-4 h-4 text-orange-600 border-gray-300 rounded focus:ring-orange-500" onchange="updateBulkDeleteBtn()">
                                </td>
                                <td class="font-bold text-gray-900">#{{ $service['id'] }}</td>
                            <td class="font-bold text-gray-900">{{ $service['name'] }}</td>
                            <td><span class="network-badge">{{ $service['network'] }}</span></td>
                            <td class="font-semibold text-gray-700">{{ $service['transit_time'] }}</td>
                            <td class="text-sm text-gray-600">{{ $service['items_allowed'] }}</td>
                            <td>
                                <div class="status-toggle-container">
                                    <label class="status-switch" title="Toggle Status">
                                        <input type="checkbox" 
                                               ...
                                               data-id="{{ $service['id'] }}"
                                               data-route="{{ route('admin.services.toggle-status', $service['id']) }}"
                                               {{ $service['status'] == 'Active' ? 'checked' : '' }}
                                               onchange="toggleStatus(this, 'service')">
                                        <span class="status-slider"></span>
                                    </label>
                                    <span class="status-label text-xs font-semibold {{ $service['status'] == 'Active' ? 'text-green-700' : 'text-gray-500' }}">
                                        {{ $service['status'] }}
                                    </span>
                                </div>
                            </td>
                            <td class="text-xs text-gray-500 max-w-xs truncate">{{ $service['remark'] ?? '-' }}</td>
                            <td>
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('admin.services.edit', $service['id']) }}" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                        </svg>
                                    </a>
                                    <button onclick="deleteService({{ $service['id'] }})" class="p-2 text-red-600 hover:bg-red-50 rounded-lg transition" title="Delete">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                        </svg>
                                    </button>
                        ...
                        <tr>
                            <td colspan="9" class="p-0">
                                <div class="empty-state">
                                    ...
                                    <h3 class="text-lg font-bold text-gray-900 mb-2">No Services Found</h3>
                                    <p class="text-gray-600 mb-4 max-w-md mx-auto">Get started by creating your first service. Click the button above to add a new service.</p>
                                    <a href="{{ route('admin.services.create') }}" class="admin-btn-primary inline-block">
                                        <div class="flex items-center gap-2">
                                            <svg class="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                                            </svg>
    ...
    <script>
        ...
        function bulkDelete() {
            ...
            if (confirm(`Are you sure you want to delete ${checkboxes.length} selected service(s)?`)) {
