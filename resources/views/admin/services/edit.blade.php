...
@section('content')
    ...
    <div id="success-popup" class="success-popup" style="display: none;">
        ...
        <div class="flex-1">
            <p class="text-green-700 font-semibold text-sm" id="success-message">Service updated successfully!</p>
        </div>
    ...
    <div class="page-header">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 mb-0.5">Edit Service</h1>
                    <p class="text-xs text-gray-600">Update service information - {{ $service['name'] }}</p>
                </div>
            </div>
            <a href="{{ route('admin.services.all') }}" class="px-5 py-2.5 rounded-xl border-2 border-gray-300 text-gray-700 font-semibold hover:bg-gray-50 transition text-sm flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="form-card p-6">
                ...
                <form id="serviceForm" method="POST" action="{{ route('admin.services.update', $service['id']) }}">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="service_name" id="service_name" class="form-input" value="{{ $service['name'] }}" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="network" id="network" class="form-select" required>
                            ...
                            @foreach($networks as $network)
                                <option value="{{ $network['name'] }}" {{ $service['network'] == $network['name'] ? 'selected' : '' }}>{{ $network['name'] }} ({{ $network['type'] }})</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="transit_time" id="transit_time" class="form-input" value="{{ $service['transit_time'] }}" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="items_allowed" id="items_allowed" class="form-input" value="{{ $service['items_allowed'] }}" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="display_title" id="display_title" class="form-input" placeholder="e.g., E-Commerce delivery, Pick & Drop" value="{{ old('display_title', $service['display_title'] ?? $service['name']) }}">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description" id="description" rows="3" class="form-textarea resize-none" placeholder="Service description to display on landing page">{{ old('description', $service['description'] ?? '') }}</textarea>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="icon-dropdown-wrapper">
                            <input type="hidden" name="icon_type" id="icon_type" value="{{ $service['icon_type'] ?? 'truck' }}">
                    ...
                    <div class="form-group">
                        ...
                        <div class="status-toggle">
                            <label class="status-switch">
                                <input type="checkbox" name="is_highlighted" id="is_highlighted" {{ ($service['is_highlighted'] ?? false) ? 'checked' : '' }}>
                                <span class="status-slider"></span>
                            </label>
                            <span class="text-sm font-medium text-gray-700" id="highlightLabel">{{ ($service['is_highlighted'] ?? false) ? 'Highlighted' : 'Not Highlighted' }}</span>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="remark" id="remark" rows="3" class="form-textarea resize-none" placeholder="Any additional notes">{{ old('remark', $service['remark'] ?? '') }}</textarea>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="status-toggle">
                            <label class="status-switch">
                                <input type="checkbox" name="status" id="status" {{ $service['status'] == 'Active' ? 'checked' : '' }}>
                                <span class="status-slider"></span>
                            </label>
                            <span class="text-sm font-medium text-gray-700" id="statusLabel">{{ $service['status'] }}</span>
                        </div>
                    </div>
        ...
        <div class="lg:col-span-1">
            <div class="form-card p-5 sticky top-6">
                ...
                <div class="space-y-3">
                    <div>
                        <p class="text-xs text-gray-500 mb-1">Service ID</p>
                        <p class="text-sm font-bold text-gray-900">#{{ $service['id'] }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        <p class="text-xs text-gray-500 mb-1">Current Network</p>
                        <p class="text-sm font-bold text-gray-900">{{ $service['network'] }}</p>
                    </div>
                    <div>
    ...
    <script>
        ...
        document.getElementById('serviceForm').addEventListener('submit', function(e) {
            ...
            .then(result => {
                if (result.redirected) {
                    ...
                    if (result.data.success) {
                        showSuccessPopup(result.data.message || 'Service updated successfully!');
                        // Redirect after a short delay
                        setTimeout(() => {
                        ...
                        if (result.data.errors) {
                            ...
                            alert(result.data.message || 'An error occurred while updating the service.');
            ...
            .catch(error => {
                ...
                alert('An error occurred while updating the service. Please check the console for details.');
                submitButton.disabled = false;
                submitButton.innerHTML = originalButtonText;
            });
        });
        ...
        const currentIconType = iconTypeInput.value || 'truck';
        populateIconOptions();
        selectIcon(currentIconType);
    </script>
@endsection
