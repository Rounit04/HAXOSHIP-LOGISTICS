...
@section('content')
    ...
    <div id="success-popup" class="success-popup" style="display: none;">
        ...
        <div class="flex-1">
            <p class="text-green-700 font-semibold text-sm" id="success-message">Network created successfully!</p>
        </div>
    ...
    <div class="page-header">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 mb-0.5">Create Network</h1>
                    <p class="text-xs text-gray-600">Add a new network - Single, bulk, editable Status (Active/Inactive)</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="form-card p-6 mb-6" style="background: linear-gradient(135deg, #fff5ed 0%, #ffe8d6 100%); border: 2px solid rgba(255, 117, 15, 0.2);">
        <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-lg font-bold text-gray-900">Bulk Import Networks</h2>
                    <p class="text-sm text-gray-600">Upload an Excel file to import multiple networks at once</p>
                </div>
            </div>
            <a href="{{ route('admin.networks.template.download') }}" class="admin-btn-primary px-4 py-2 text-sm">
        ...
        <form method="POST" action="{{ route('admin.networks.import') }}" enctype="multipart/form-data" class="flex items-center gap-3">
            @csrf
            ...
            <div class="form-card p-6">
                ...
                <form id="networkForm" method="POST" action="{{ route('admin.networks.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="opening_balance" id="opening_balance" step="0.01" min="0" class="form-input" placeholder="0.00" value="0.00" required>
                    </div>
            ...
            <div class="form-card p-5 sticky top-6">
                ...
                <div class="space-y-3 text-xs text-gray-600">
                    <div class="flex items-start gap-2">
                        ...
                        <span>Network Name must be unique</span>
                    </div>
                    <div class="flex items-start gap-2">
                        <div class="w-1.5 h-1.5 rounded-full bg-purple-600 mt-1.5 flex-shrink-0"></div>
                        <span>Choose Domestic or International based on coverage area</span>
                    </div>
                    <div class="flex items-start gap-2">
                        <div class="w-1.5 h-1.5 rounded-full bg-purple-600 mt-1.5 flex-shrink-0"></div>
                        <span>Opening Balance should be set to initial account balance</span>
                    </div>
                    <div class="flex items-start gap-2">
                        <div class="w-1.5 h-1.5 rounded-full bg-purple-600 mt-1.5 flex-shrink-0"></div>
                        <span>Status can be changed later from All Networks</span>
                    </div>
                </div>
    ...
    <script>
        ...
        document.getElementById('networkForm').addEventListener('submit', function(e) {
            ...
            if (!networkName || !networkType || !openingBalance) {
                alert('Please fill in all required fields (Network Name, Network Type, and Opening Balance).');
                return;
            }
            ...
            console.log('Status checkbox checked:', statusCheckbox.checked);
            ...
            .then(result => {
                if (result.redirected) {
                    // Show success popup
                    showSuccessPopup('Network created successfully!');
                    ...
                    if (result.data.success) {
                        showSuccessPopup(result.data.message || 'Network created successfully!');
                        // Reset form
                        form.reset();
                        ...
                        if (result.data.errors) {
                            ...
                            alert(result.data.message || 'An error occurred while creating the network.');
            ...
            .catch(error => {
                ...
                alert('An error occurred while creating the network. Please check the console for details.');
                submitButton.disabled = false;
                submitButton.innerHTML = originalButtonText;
            });
        });
        ...
        function switchUpiMode(mode) {
            ...
            if (mode === 'text') {
                ...
                textBtn.classList.remove('border-gray-300', 'text-gray-700', 'bg-white');
                ...
                fileBtn.classList.add('border-gray-300', 'text-gray-700', 'bg-white');
                ...
                // Update button styles
                fileBtn.classList.remove('border-gray-300', 'text-gray-700', 'bg-white');
                ...
                textBtn.classList.add('border-gray-300', 'text-gray-700', 'bg-white');
        ...
        function previewUpiImage(input) {
            ...
            const fileInput = document.getElementById('upi_scanner_file_value');
            ...
            if (input.files && input.files[0]) {
                ...
                if (fileInput) {
                    fileInput.value = input.files[0].name;
                }
            }
        }
