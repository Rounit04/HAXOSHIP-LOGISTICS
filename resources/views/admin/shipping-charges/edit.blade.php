...
@section('content')
    ...
    <div id="success-popup" class="success-popup" style="display: none;">
        ...
        <div class="flex-1">
            <p class="text-green-700 font-semibold text-sm" id="success-message">Shipping charge updated successfully!</p>
        </div>
    ...
    <div class="page-header">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 mb-0.5">Edit Shipping Charge</h1>
                    <p class="text-xs text-gray-600">Update shipping charge - ID: #{{ $charge['id'] }}</p>
                </div>
            </div>
            <a href="{{ route('admin.shipping-charges.all') }}" class="px-5 py-2.5 rounded-xl border-2 border-gray-300 text-gray-700 font-semibold hover:bg-gray-50 transition text-sm flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            <div class="form-card p-6">
                <h2 class="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
                    ...
                    Update Shipping Charge Information
                </h2>

                <form id="shippingChargeForm" method="POST" action="{{ route('admin.shipping-charges.update', $charge['id']) }}">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="form-group">
                            ...
                            <select name="origin" id="origin" class="form-select" required>
                                ...
                                @foreach($countries as $country)
                                    <option value="{{ $country['name'] }}" {{ $charge['origin'] == $country['name'] ? 'selected' : '' }}>{{ $country['name'] }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                    ...
                    <!-- Destination, Destination Pincode & Destination Zone -->
                    <div class="form-grid">
                        <div class="form-group">
                            <label class="form-label">
                                <svg class="w-3.5 h-3.5 text-orange-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <select name="destination" id="destination" class="form-select" required>
                                ...
                                @foreach($countries as $country)
                                    <option value="{{ $country['name'] }}" {{ $charge['destination'] == $country['name'] ? 'selected' : '' }}>{{ $country['name'] }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <select name="shipment_type" id="shipment_type" class="form-select" required>
                            ...
                            @foreach($shipmentTypes as $type)
                                <option value="{{ $type }}" {{ $charge['shipment_type'] == $type ? 'selected' : '' }}>{{ $type }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="number" name="min_weight" id="min_weight" class="form-input" step="0.01" min="0.01" value="{{ $charge['min_weight'] }}" required>
                        </div>
                        <div class="form-group">
                            <label class="form-label">
                                <svg class="w-3.5 h-3.5 text-orange-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <input type="number" name="max_weight" id="max_weight" class="form-input" step="0.01" min="0.01" value="{{ $charge['max_weight'] }}" required>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="network" id="network" class="form-select" required>
                                ...
                                @foreach($networks as $network)
                                    <option value="{{ $network['name'] }}" {{ $charge['network'] == $network['name'] ? 'selected' : '' }}>{{ $network['name'] }} ({{ $network['type'] }})</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="service" id="service" class="form-select" required>
                                ...
                                @foreach($services as $service)
                                    @if($service['network'] == $charge['network'])
                                        <option value="{{ $service['name'] }}" {{ $charge['service'] == $service['name'] ? 'selected' : '' }}>{{ $service['name'] }}</option>
                                    @endif
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="rate" id="rate" class="form-input" step="0.01" min="0" value="{{ $charge['rate'] }}" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="remark" id="remark" rows="3" class="form-textarea resize-none" placeholder="Any additional notes">{{ $charge['remark'] ?? '' }}</textarea>
                    </div>
                    ...
                    <div class="flex gap-3">
                        <button type="submit" class="admin-btn-primary px-6 py-3 text-sm font-semibold flex-1">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <span>Update Shipping Charge</span>
                            </div>
                        </button>
        ...
        <div class="lg:col-span-1">
            <div class="form-card p-5 sticky top-6">
                ...
                <div class="space-y-3">
                    <div>
                        <p class="text-xs text-gray-500 mb-1">Charge ID</p>
                        <p class="text-sm font-bold text-gray-900">#{{ $charge['id'] }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 mb-1">Origin → Destination</p>
                        <p class="text-sm font-bold text-gray-900">{{ $charge['origin'] }} → {{ $charge['destination'] }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 mb-1">Zones</p>
                        <p class="text-sm font-bold text-gray-900">{{ $charge['origin_zone'] }} → {{ $charge['destination_zone'] }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 mb-1">Shipment Type</p>
                        <p class="text-sm font-bold text-gray-900">{{ $charge['shipment_type'] }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 mb-1">Weight Range</p>
                        <p class="text-sm font-bold text-gray-900">{{ number_format($charge['min_weight'], 2) }} - {{ number_format($charge['max_weight'], 2) }} KG</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-500 mb-1">Current Rate</p>
                        <p class="text-sm font-bold text-green-600">₹{{ number_format($charge['rate'], 2) }}</p>
                    </div>
                </div>
    ...
    <script>
        ...
        const currentChargeNetwork = '{{ $charge['network'] }}';
        const currentChargeService = '{{ $charge['service'] }}';
        ...
        function populateServices() {
            const selectedNetwork = networkSelect.value || currentChargeNetwork;
            ...
            if (selectedNetwork) {
                ...
                if (filteredServices.length > 0) {
                    filteredServices.forEach(service => {
                        ...
                        if (service.name === currentChargeService && selectedNetwork === currentChargeNetwork) {
        ...
        const currentOriginPincode = '{{ $charge["origin_pincode"] ?? "" }}';
        const currentOriginZone = '{{ $charge["origin_zone"] ?? "" }}';
        const currentDestinationPincode = '{{ $charge["destination_pincode"] ?? "" }}';
        const currentDestinationZone = '{{ $charge["destination_zone"] ?? "" }}';
        ...
        function updateZones(zoneSelectElement, pincodeSelectElement, selectedZone = '') {
        ...
        document.getElementById('origin')?.addEventListener('change', function() {
            updatePincodes(document.getElementById('origin_pincode'), this.value, currentOriginPincode);
        ...
        document.getElementById('origin_pincode')?.addEventListener('change', function() {
            updateZones(document.getElementById('origin_zone'), this, currentOriginZone);
        });

        // Destination country change
        document.getElementById('destination')?.addEventListener('change', function() {
            updatePincodes(document.getElementById('destination_pincode'), this.value, currentDestinationPincode);
        ...
        document.getElementById('destination_pincode')?.addEventListener('change', function() {
            updateZones(document.getElementById('destination_zone'), this, currentDestinationZone);
        });

        // Load pincodes and zones on page load if countries are already selected
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (originCountry) {
                updatePincodes(document.getElementById('origin_pincode'), originCountry, currentOriginPincode).then(() => {
                    if (currentOriginPincode) {
                        updateZones(document.getElementById('origin_zone'), document.getElementById('origin_pincode'), currentOriginZone);
                    }
                });
            }
            if (destinationCountry) {
                updatePincodes(document.getElementById('destination_pincode'), destinationCountry, currentDestinationPincode).then(() => {
                    if (currentDestinationPincode) {
                        updateZones(document.getElementById('destination_zone'), document.getElementById('destination_pincode'), currentDestinationZone);
                    }
                });
            }
        });
        ...
        document.getElementById('shippingChargeForm').addEventListener('submit', function(e) {
            ...
            .then(result => {
                if (result.redirected) {
                    // Show success popup
                    showSuccessPopup('Shipping charge updated successfully!');
                    ...
                    if (result.data.success) {
                        showSuccessPopup(result.data.message || 'Shipping charge updated successfully!');
                        // Redirect after a short delay
                        setTimeout(() => {
                        ...
                        if (result.data.errors) {
                            ...
                            alert(result.data.message || 'An error occurred while updating the shipping charge.');
            ...
            .catch(error => {
                ...
                alert('An error occurred while updating the shipping charge. Please check the console for details.');
                submitButton.disabled = false;
                submitButton.innerHTML = originalButtonText;
            });
        });
