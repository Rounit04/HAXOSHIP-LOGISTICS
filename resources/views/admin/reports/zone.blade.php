...
@section('content')
    ...
    <div class="page-header">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.reports.zone.export', $exportParams) }}" class="admin-btn-primary px-5 py-2.5 text-sm font-semibold flex items-center gap-2">
                    <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="report-card p-6">
        ...
        <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4 mb-4">
            <form method="GET" action="{{ route('admin.reports.zone') }}" class="flex flex-wrap items-end gap-3">
                <div>
                    <label class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1 block">Search</label>
                    <input type="text" name="search" class="form-input" placeholder="Search by pincode, country, zone, network..." value="{{ $search ?? request('search') }}">
                </div>
                <div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    @if(($dateFrom ?? request('date_from')) || ($dateTo ?? request('date_to')) || ($search ?? request('search')))
                        <a href="{{ route('admin.reports.zone') }}" class="px-4 py-2 rounded-lg border border-gray-300 text-gray-600 text-sm font-semibold hover:bg-gray-50 transition">
                            Reset
                        </a>
                    @endif
                </div>
