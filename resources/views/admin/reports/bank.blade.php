...
@section('content')
    ...
    <div class="report-card p-6">
        ...
        <div class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4 mb-4">
            <form method="GET" action="{{ route('admin.reports.bank') }}" class="flex flex-wrap items-end gap-3">
                <div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    @if(($dateFrom ?? request('date_from')) || ($dateTo ?? request('date_to')))
                        <a href="{{ route('admin.reports.bank') }}" class="px-4 py-2 rounded-lg border border-gray-300 text-gray-600 text-sm font-semibold hover:bg-gray-50 transition">
                            Reset
                        </a>
                    @endif
                </div>
