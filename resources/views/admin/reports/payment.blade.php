    ...
    <div class="page-header">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 mb-0.5">Payment Report</h1>
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.reports.payment.export', $exportParams) }}" class="admin-btn-primary px-5 py-2.5 text-sm font-semibold flex items-center gap-2">
                    <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="report-card p-6">
        <div class="flex items-center justify-between mb-4">
            ...
            <div class="text-sm text-gray-600 font-medium">
                Total: <span class="font-bold text-orange-600">{{ count($payments) }}</span> Payments
            </div>
        </div>
        ...
        <div class="overflow-x-auto rounded-lg border border-gray-200">
                    ...
                    <tbody>
                        @foreach($payments as $payment)
                            <tr>
                                ...
                                <td><span class="status-badge {{ strtolower($payment['status'] ?? '') == 'completed' ? 'active' : 'inactive' }}">{{ $payment['status'] ?? 'Pending' }}</span></td>
                                <td class="text-sm text-gray-600">{{ $payment['reference'] ?? '-' }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            @else
                <div class="text-center py-12">
                    ...
                    <h3 class="text-lg font-semibold text-gray-900 mb-2">No Payment Data Found</h3>
                    <p class="text-gray-600 text-sm">No payment data available to display in the report.</p>
                </div>
            @endif
        </div>
    </div>
