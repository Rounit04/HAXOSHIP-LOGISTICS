...
@section('content')
    ...
    <div class="page-header">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 mb-0.5">Add Payment</h1>
                    <p class="text-xs text-gray-600">Single payment entry</p>
                </div>
            </div>
        </div>
    </div>
                ...
                <form method="POST" action="{{ route('admin.payments.store') }}">
                    ...
                    <div class="form-group">
                        ...
                        <select name="payment_to" id="payment_to" class="form-select" required onchange="updateCategories()">
                            ...
                            <option value="network" {{ old('payment_to') == 'network' ? 'selected' : '' }}>Network</option>
                            <option value="expense" {{ old('payment_to') == 'expense' ? 'selected' : '' }}>Expense</option>
                            <option value="income" {{ old('payment_to') == 'income' ? 'selected' : '' }}>Income</option>
                        </select>
                        @error('payment_to')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="remark" id="remark" rows="3" class="form-textarea resize-none" placeholder="Enter payment remarks or notes (optional)">{{ old('remark') }}</textarea>
                        @error('remark')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
    ...
    <script>
        ...
        const oldPaymentTo = '{{ old("payment_to") }}';
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // If payment_to is already selected (from old input), update categories
