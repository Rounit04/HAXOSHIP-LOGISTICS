...
@section('title', 'All Direct Entry Entries')
...
@section('content')
    ...
    <div class="page-header">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 mb-0.5">All Direct Entry Entries</h1>
                    <p class="text-xs text-gray-600">Single and Bulk Both Required</p>
                </div>
            </div>
            <a href="{{ route('admin.direct-entry.create') }}" class="admin-btn-primary px-5 py-2.5 text-sm">
                <div class="flex items-center gap-2">
                    <svg class="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                    </svg>
                    <span>Add Direct Entry</span>
                </div>
            </a>
        </div>
    </div>
    ...
    <div class="search-section">
        ...
        <form method="GET" action="{{ route('admin.direct-entry.all') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <!-- AWB/Origin/Destination Search -->
            <div>
                ...
                <input type="text" name="search" value="{{ $searchParams['search'] ?? '' }}" class="search-input" placeholder="Search by AWB, origin, or destination">
            </div>
            ...
            <div class="flex items-end gap-2">
                ...
                @if($searchParams['search'] ?? '' || $searchParams['network'] ?? '' || $searchParams['shipment_type'] ?? '')
                    <a href="{{ route('admin.direct-entry.all') }}" class="px-4 py-2.5 rounded-lg border-2 border-gray-300 text-gray-700 font-semibold hover:bg-gray-50 transition text-sm">
                        Clear
                    </a>
                @endif
            </div>
    ...
    <div class="booking-card p-6">
        <div class="flex items-center justify-between mb-4">
            ...
            <div class="flex items-center gap-4">
                <div class="text-sm text-gray-600 font-medium">
                    Total: <span class="font-bold text-orange-600">{{ count($bookings) }}</span> Entries
                </div>
        ...
        <form id="bulkDeleteForm" method="POST" action="{{ route('admin.direct-entry.bulk-delete') }}">
            @csrf
                    ...
                    <table class="booking-table min-w-full">
                        ...
                        <tbody>
                            @foreach($bookings as $booking)
                                ...
                                <td>{{ number_format($booking['chr_weight'], 2) }} KG</td>
                                <td>{{ $booking['pieces'] }}</td>
                                <td>{{ $booking['network'] ?? 'N/A' }}</td>
                                <td>{{ $booking['service'] ?? 'N/A' }}</td>
                                <td class="font-semibold text-green-600">₹{{ number_format($booking['booking_amount'], 2) }}</td>
                                <td>
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('admin.direct-entry.edit', $booking['id']) }}" class="px-3 py-1.5 bg-blue-500 text-white text-xs font-semibold rounded-lg hover:bg-blue-600 transition-colors">
                                            Edit
                                        </a>
                                        <form action="{{ route('admin.direct-entry.delete', $booking['id']) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this entry?');">
                                            @csrf
                                            @method('DELETE')
            ...
            @else
                <div class="empty-state">
                    ...
                    <h3 class="text-lg font-semibold text-gray-900 mb-2">No Direct Entry Entries Found</h3>
                    <p class="text-gray-600 text-sm mb-4">Get started by creating your first direct entry.</p>
                    <a href="{{ route('admin.direct-entry.create') }}" class="admin-btn-primary px-6 py-2.5 text-sm inline-flex items-center gap-2">
                        <svg class="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                        </svg>
                        Add Direct Entry
    ...
    <script>
        ...
        function bulkDelete() {
            ...
            if (confirm(`Are you sure you want to delete ${checkboxes.length} selected entry/entries?`)) {
